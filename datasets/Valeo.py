...
class ValeoDataset(Dataset):
    """
    Class to handle Valeo dataset for segmentation task.
    """
    ...
    def load_subsampled_clouds(self, subsampling_parameter):
        ...
        self.test_files = self.test_files[[2, 3, -3, -2]]
        #self.test_files = self.test_files[[2, 3]]
        ...
        for i, file_path in enumerate(files):
            ...
            if isfile(KDTree_file):
                ...
                reflectance = np.zeros_like(points[:, :1])
                if cloud_split == 'test':
                    int_features = None
                else:
    ...
    def get_batch_gen(self, split, config):
        ...
        if not hasattr(self, 'potentials'):
            self.potentials = {}
            self.min_potentials = {}
            self.potentials_order = {}

        # Reset potentials
        self.potentials[split] = []
        self.min_potentials[split] = []
        self.potentials_order[split] = []
        for i, tree in enumerate(self.input_colors[split]):
            ...
            self.potentials_order[split] += [np.argsort(self.potentials[split][-1])]
        ...
        def spatially_regular_gen_new():
            ...
            for i in range(epoch_n):
                ...
                point_ind = self.potentials_order[split][cloud_ind][0]
                ...
                # Merge sorted potentials and new potential values
                other_potentials = np.delete(self.potentials[split][cloud_ind], input_inds)
                other_potentials_order = np.delete(self.potentials_order[split][cloud_ind], input_inds)

                new_potential_order = np.argsort(self.potentials[split][cloud_ind][input_inds])
                new_potential_sorted = self.potentials[split][cloud_ind][input_inds][new_potential_order]

                insert_inds = np.searchsorted(other_potentials,
                                              new_potential_sorted,
                                              sorter=other_potentials_order)
                ...
                print('New element : ', (np.array(t[1:]) - np.array(t[:-1]))*1000)


                # In case batch is full, yield it and reset it
                if batch_n + n > self.batch_limit and batch_n > 0:
                    print('yielding a new bacth')
                    yield (np.concatenate(p_list, axis=0),
                           np.concatenate(c_list, axis=0),
                           np.concatenate(pl_list, axis=0),
                           np.array([tp.shape[0] for tp in p_list]),
